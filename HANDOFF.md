...
- **他の人にも同じ通知を送る**（2026-09-11）: 相手が BotFather で Bot を作り → その Bot に1通送り → トークンを非公開で受け取る → `getUpdates` で chat_id を取る → 4リポの Secret `TG_EXTRA` に `[{"token":"…","chat":"…"}]` を登録（`whale_repo/set_secret.py`、PyNaCl 必要）。`tg()` は主宛先＋TG_EXTRA の全宛先に独立して送る。トークンは公開リポの config.json には絶対に書かない。人を外す＝TG_EXTRA から削る
