...
- 🟢 買い ≥$50,000 即時（2026-09-11 に $5,000 から引き上げ。表示は枚数・合計額・総資産比・時刻・回数・平均取得→いま・流動性・DexScreener、支払い内訳は出さない）。実行をまたぐ分割買いは `accumulate_buys` が `state.buy_accum` に 24h 累計し、$50,000 に達した回で「（分割買いの累計）」付きで通知。台帳の買い一覧・新規銘柄も $5,000 以上のみ（`buy_list_min_usd`、$1,000 は藤沼さんが「低すぎる」）
