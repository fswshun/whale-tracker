...
- 🟢 買い ≥$50,000 即時（2026-09-11 に $5,000 から引き上げ。表示は枚数・合計額・総資産比・時刻・回数・平均取得→いま・流動性・DexScreener、支払い内訳は出さない）。実行をまたぐ分割買いは `accumulate_buys` が `state.buy_accum` に 24h 累計し、$50,000 に達した回で「（分割買いの累計）」付きで通知。台帳の買い一覧・新規銘柄の成績も同じ $50,000（2026-09-13 に一本化、旧 `buy_list_min_usd` は廃止。通知と台帳で数字がずれると混乱するため）。銘柄別の日次推移は左が最新・右が過去（前日比と累計の計算は時系列のまま）
- 📥 大口受取 ≥$100,000 即時（素の「受取」のみ、異常トークンは valuation_check で除外。EMBER $621K が受取で着弾して買い通知が無かった件から。買いには数えない）
...
- **tx 内ネット集計（2026-09-11）**: `group_trades` は tx 内で銘柄ごとに IN−OUT を差し引いてから集計する。アグリゲータの経路上の中間トークン（USDC→SOL→EMBER の SOL、MET の往復）は ネット 0 で無視。以前は tx 内の全 OUT 足を各 IN 足に丸ごと付けていたため、中間トークン MET が「$33K の買い」として誤通知され、EMBER の買い合計も $101K と二重計上されていた（実際は約 $50K）
...
- **2026-09-23 の6日間停止（再発注意）**: NodeReal の月間無料枠が切れ、HTTP 429 + `code -32005` を返すようになった。`http_json` が 429 を一律に「待てば直る」と見なして 1 回 36 秒の再試行を繰り返し、BSC を持つウォレットの数だけ積み上がって実行が 25 分の上限に到達。GitHub の concurrency（`cancel-in-progress: false`）で後続がキューに溜まり、次が来るたび前のキューがキャンセルされる連鎖になり、**Nachi は 9/20、unipcs は 9/17 からデータが 1 件も保存されなくなった**（avast・kyle は Alchemy 経由で軽いため無事）。対策 = ①枠切れ文言（quota / -32005 / exceeded 等）の 429 は即諦めてそのホストをこの実行では叩かない ②NodeReal 枠切れを検知したら Alchemy に自動で乗り換え（`_nodereal_dead`） ③実行の時間予算を 18 分 → 8 分 ④GitHub の `schedule` を廃止して起動は cron-job.org 一本 ⑤`cancel-in-progress: true` にして詰まった実行は捨てる ⑥timeout 25 分 → 15 分。**NodeReal の枠は月初にリセットされる。それまで Nachi と unipcs は Alchemy キー（avast と共用）で動く。10 月に入ったら NodeReal に戻るので共用は解消してよい**
- **Helius の Enhanced API は健在**（2026-09-23 に全バッチサイズで確認）。一時的に 400 や 429 を返すことがあるだけ
