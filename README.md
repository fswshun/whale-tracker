...
すべての通知の先頭に `【クジラ名】` が付く（名前の無い第1クジラは「メインクジラ」）。

| 種類 | タイミング | 内容 |
|---|---|---|
| 🟢 買い | 検出した実行で即時（10分ごとの実行＋GitHub の遅延で、オンチェーンから 5〜20 分後） | `buy_alert_usd`（既定 $5,000）以上のリスク資産の買い。同一銘柄の分割買いは1行に集約。支払い元・平均取得・DexScreener リンク付き |
| 🔻 売り / 📤 外部流出 | 即時 | `sell_alert_usd`（既定 $100,000）以上の目立つ売り・クラスター外への流出 |
