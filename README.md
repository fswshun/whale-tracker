...
クジラウォレット群（本体＋自動検出した子・孫）の資産を 10 分ごとに追跡し、**買い（$50,000〜）と目立つ売り・大口受取（$100,000〜）は即時 Telegram**、1日1通の日次レポート（24:00 JST）で総資産の推移と大きな動きを通知、GitHub Pages に資産レポートを公開します。
...
| 種類 | タイミング | 内容 |
|---|---|---|
| 🟢 買い | 検出した実行で即時（10分ごとの実行＋GitHub の遅延で、オンチェーンから 5〜20 分後） | `buy_alert_usd`（既定 $50,000）以上のリスク資産の買い。同一銘柄の分割買いは1行に集約し、**実行をまたぐ分割買いも 24 時間（`buy_accum_hours`）累計して $50,000 に達した時点で通知**（$10,000 × 5 回など）。表示は 枚数・合計額・総資産比・JST 時刻・回数・平均取得と現在価格の比・流動性・DexScreener（支払いの内訳は出さない） |
| 🔻 売り | 即時 | `sell_alert_usd`（既定 $100,000）以上の目立つ売り。クラスター外への流出（取引所等）は以後追えず売却リスクが高いので**売り扱い**。買い・売りとも「総資産の何%か」を併記（10% 以上は「大口」） |
| 📥 大口受取 | 即時 | `sell_alert_usd`（既定 $100,000）以上のリスク資産がクラスター外から届いたもの（取引所からの引き出し・購入の着弾など。買いには数えない。異常トークンは除く） |
